...
static void handle_navigation(char *);
...
static void handle_navigation(char *destination)
{
    if (strcmp(destination, "Appointment") == 0)
        display_choose_problems_window();
    else if (strcmp(destination, "Doctor") == 0)
        display_choose_doctor_window();
    else if (strcmp(destination, "Service") == 0)
...
static void display_user_details_input_window(GtkWidget *widget, gpointer data)
{
    ...
    int is_appointment = strcmp((char *)data, "Appointment");
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(validate_user_data), (char *)data);

    gtk_grid_attach(GTK_GRID(outer_grid), button, 0, is_appointment == 0 ? 2 : 1, 2, 1);
...
static void display_appointment_booked_window(GtkWidget *widget, gpointer disease_type)
{
    GtkWidget *grid;
    GtkWidget *label;
    GtkWidget *button;
    ...
    BookingData *disease = (BookingData *)disease_type;
    ...
    else if (disease->type == appointment)
    {
        char date_string[40] = "Appointment Booked for ";
    ...
    label = gtk_label_new(disease->data);
...
static void validate_user_data(GtkWindow *parent, char *message)
{
    ...
    if (strcmp(message, "Appointment") == 0)
    {
        if (valid)
            g_print("Valid Date\n");
        else
